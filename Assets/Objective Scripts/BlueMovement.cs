...
public class BlueMovement : MonoBehaviour {
	...
	void Update () {
		...
		if ((SixenseInput.Controllers[1].JoystickX < 0) && nearBuild == false) {
			blu.gameObject.transform.RotateAround(Vector3.zero, Vector3.down, 15 * -(SixenseInput.Controllers[1].JoystickX)*Time.deltaTime);
		}
		if ((SixenseInput.Controllers[1].JoystickX > 0) && nearBuildOpp == false) {
			blu.gameObject.transform.RotateAround(Vector3.zero, Vector3.up, 15 *(SixenseInput.Controllers[1].JoystickX)*Time.deltaTime);
		}
		if ((SixenseInput.Controllers[1].JoystickY > 0) && far == true && nearBuild == false && nearBuildOpp == false) {
			blu.gameObject.transform.Translate(transform.forward * (Time.deltaTime * -25 * (SixenseInput.Controllers[1].JoystickY)),Space.World);
		...
		if ((SixenseInput.Controllers[0].JoystickY > 0) && top == true) {
			blu.gameObject.transform.localPosition += transform.up * 30 * Time.deltaTime;
		}
